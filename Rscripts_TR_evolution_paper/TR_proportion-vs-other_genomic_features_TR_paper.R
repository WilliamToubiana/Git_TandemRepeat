...
###Fitted Generalized Additive Models
library(mgcv)

#GAMs 
merge_sub$chr<-as.factor(merge_sub$chr)

model<-gam(TRprop~chr + s(median_rho) + s(start2.x, by=chr), data=merge_sub, family = betar(),  method = "REML")
summary(model)

#Fitted genomic landscapes
merge_sub2<-merge_sub #across all 12 chromosomes
merge_sub2<-subset(merge_sub, chr=="Tps_LRv5b_scf1") #per chromosomes
...
#GAMs 
merge2$chr<-as.factor(merge2$chr)

model<-gam(TRprop~chr + s(perc_GC) + s(start2.x, by=chr), data=merge2, family = betar(),  method = "REML")
summary(model)


#Fitted genomic landscapes

merge_sub<-merge2 #across all 12 chromosomes
merge_sub<-subset(merge2, chr=="Tps_LRv5b_scf1")#per chromosomes

library(mgcv)
gamGC<-gam(perc_GC ~ s(cumul_start, bs = "cs", k=30), data=merge_sub)

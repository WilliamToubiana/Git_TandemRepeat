...
Tps_uniq<-read.table("Tps_LRv5b_mtDNAv350.fasta.2.7.7.80.10.50.2000_parse_5copies_CMwRC.txt", header=T, sep="\t", quote="")
...
ggplot(data.frame(Tps_uniq), aes(x=Count_LGs)) + 
  geom_bar()+
  theme_minimal()
...
###Tandem repeat proportion between chromosomes
setwd("/Users/wtoubian/Desktop/Tandem_repeats/Timema_evolution/TR_annotation_timema/minimal_rotations")
...
nonoverlapWind_chr_sum<-aggregate(nonoverlapWind_chr$count~nonoverlapWind_chr$seqnames, FUN=sum)
...
table$motif_count<-nonoverlapWind_chr_sum$`nonoverlapWind_chr$count`
...
TR_overlap_windows<-read.table("TR_nonoverlapWind_chr_intersection10kbWind_10kbWindows.txt", header=F, sep="\t", quote="")
...
windows<-read.table("Tps_chm_size_mtDNAv350_w10000.bed", header=F, sep="\t", quote="")
...
h3k9<-read.table("Tps_testes_h3k9_coverage_DR_10kb.txt",
                  header=FALSE)

input<-read.table("Tps_testes_input1_coverage_DR_10kb.txt",
                   header=FALSE)
...
chms7$cumul_start<-seq(0, by = 10000, length.out = nrow(chms7))
...
################################################################
# Pair-wise (sex-asex sister species) TR proportion comparison #
################################################################
...
data2<-rbind(do.call("rbind", list(Tps_subset, Tdi_subset, Tcm_subset, Tsi_subset, Tce_subset, Tms_subset, 
...
data2$region_size<-data2$copy_nb*data2$motif_length
data2$propTR<-data2$region_size/data2$genome

ggplot(data2, aes(x=species, y=motif_length)) + ylim(0,50) +
  geom_violin(trim=FALSE) + geom_boxplot(width=0.05)

ggplot(data2, aes(x=species, y=copy_nb)) + ylim(0,50) +
  geom_violin(trim=FALSE) + geom_boxplot(width=0.05)

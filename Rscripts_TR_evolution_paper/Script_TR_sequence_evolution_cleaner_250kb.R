...
presence_absence_df_Tps$shared<-rowSums( presence_absence_df_Tps[,c(1,2,3,4,5)])#shared without Tdi
...
combined4<-merge(combined3, data_GC[c(4,6)], by="chm_pos", all.x = T)
summary(combined4)
...
#Stats
model<-lm(combined4_subset2$avg_shared~combined4_subset2$TRprop*combined4_subset2$centromere)
anova(model)
...
ggplot(combined4_subset2, aes(x=centromere, y=avg_shared)) + 
  geom_violin()+
  ylab("Avg. nb of species sharing TR sequences")+
  theme_minimal()+
...
#filtering extreme
combined3_subset3 <- combined3_subset2 %>%
...
combined4_subset_subset<-subset(combined4_subset, chr=="Tps_LRv5b_scf1" | chr=="Tps_LRv5b_scf2" | chr=="Tps_LRv5b_scf3" | chr=="Tps_LRv5b_scf4" | chr=="Tps_LRv5b_scf5"
                                 | chr=="Tps_LRv5b_scf6" | chr=="Tps_LRv5b_scf7" | chr=="Tps_LRv5b_scf8" | chr=="Tps_LRv5b_scf9" | chr=="Tps_LRv5b_scf10"
                                 | chr=="Tps_LRv5b_scf11" | chr=="Tps_LRv5b_scf12")

combined4_subset_subset$scaff_number<-as.numeric(as.character(gsub("^.*scf","", combined4_subset_subset$chr)))

combined4_subset_subset<-combined4_subset_subset[order(combined4_subset_subset$scaff_number, combined4_subset_subset$start2),]
combined4_subset_subset$cumul_start<-seq(0, by = 250000, length.out = nrow(combined4_subset_subset))




TR_turnover<-ggplot(combined4_subset_subset, aes(x=cumul_start, y=avg_shared, color = as.factor(scaff_number)))+
  geom_point(size=0.5) +
  scale_color_manual(values = rep(c("black", "grey"), length(levels(as.factor(combined4_subset_subset$scaff_number)))/2))+
  ylab("Avg. Number of Species Sharing TR Sequences")+
...
cenh3_testes<-ggplot(combined4_subset_subset, aes(x=cumul_start, y=log2cenH3_norm, color = as.factor(scaff_number)))+
  geom_point(size=0.5) +
  scale_color_manual(values = rep(c("black", "grey"), length(levels(as.factor(combined4_subset_subset$scaff_number)))/2))+
...
TR_array<-ggplot(combined4_subset_subset, aes(x=cumul_start, y=TRprop, color = as.factor(scaff_number)))+
  geom_point(size=0.5) +
  scale_color_manual(values = rep(c("black", "grey"), length(levels(as.factor(combined4_subset_subset$scaff_number)))/2))+

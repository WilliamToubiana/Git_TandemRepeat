...
combined5<-merge(combined4, mergeTpsTcm[c(6,10,11)], by="chm_pos", all.x=T)
summary(combined5)
...
xdensity <- ggplot(combined6_subset2,
...
ydensity <- ggplot(combined6_subset2,
...
combined6_subset2$prop2 <- (combined6_subset2$prop * (n - 1) + 0.5) / n #shifts "0" into tiny positive values and "1" into slightly below 1 values 

library(glmmTMB)
model <- glmmTMB(prop2 ~ centromere * TRprop + (1|chr),
     family = beta_family(),
     data = combined6_subset2)

summary(model)


#shared TR sequences (Tps-Tcm)
combined6_subset2$centromere <- as.factor(combined6_subset2$centromere)
combined6_subset2$centromere <- relevel(
  combined6_subset2$centromere,
  ref = "non-centromere")
...
n <- nrow(combined6_subset2)
combined6_subset2$proportion_shared2 <- (combined6_subset2$proportion_shared * (n - 1) + 0.5) / n #shifts "0" into tiny positive values and "1" into slightly below 1 values 
...
combined6_subset2$TRcat <- cut(
  combined6_subset2$TRprop,
  breaks = c(0, 1/3, 2/3, 1),
  labels = c("low", "moderate", "high"),
  include.lowest = TRUE)

combined6_subset3<-combined6_subset2[!is.na(combined6_subset2$TRcat),]

ggplot(combined6_subset3,
       aes(x = TRcat,
           y = log2ratio_norm,
           fill = centromere)) +
...
combined6_subset<-subset(combined6_subset, chr!="Tps_mtDNA_v350")
summary(combined6_subset)
...
ggplot(combined6_subset2,aes(log10(median_rho), avg_motif)) + 
  geom_point() + 
  geom_smooth(method=lm) +
...
model<-lm(combined6_subset2$avg_motif~log10(combined6_subset2$median_rho))
summary(model)


ggplot(combined6_subset2,aes(log10(median_rho), median_motif)) + 
  geom_point() + 
  geom_smooth(method=lm) +
  scale_y_continuous(limits = c(0, NA))+
  theme_classic(base_size = 17)+
...
model<-lm(combined6_subset2$median_motif~log10(combined6_subset2$median_rho))
summary(model)


ggplot(combined6_subset2,aes(log10(median_rho), mean_CN)) + 
  geom_point() + 
  geom_smooth(method=lm) +
  theme_classic(base_size = 17)+
  scale_y_continuous(limits = c(0, NA))+
...
model<-lm(combined6_subset2$mean_CN~log10(combined6_subset2$median_rho))
summary(model)


ggplot(combined6_subset2,aes(log10(median_rho), log10(median_CN))) + 
  geom_point() + 
  geom_smooth(method=lm) +
  theme_classic(base_size = 17)+
  scale_y_continuous(limits = c(0, NA))+
...
model<-lm(log10(combined6_subset2$median_CN)~log10(combined6_subset2$median_rho))
summary(model)


ggplot(combined6_subset2,aes(log10(median_rho), array_density)) + 
  geom_point() + 
  geom_smooth(method=lm) +
  scale_y_continuous(limits = c(0, NA))+
  theme_classic(base_size = 17)+ 
...
model<-lm(log10(combined6_subset2$array_density)~log10(combined6_subset2$median_rho))
summary(model)


combined6_subset2$motif_diversity_corrected<-combined6_subset2$motif_diversity/combined6_subset2$total_repeats
ggplot(combined6_subset2,aes(log10(median_rho), log10(motif_diversity_corrected))) + 
  geom_point() + 
  geom_smooth(method=lm) +
  theme_classic(base_size = 17)+ 
...
model<-lm(log10(combined6_subset2$motif_diversity_corrected)~log10(combined6_subset2$median_rho))
summary(model)

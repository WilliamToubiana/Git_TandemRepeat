...
rho<-ggplot(merge_sub, aes(x=cumul_start, y=median_rho, color = as.factor(scaff_number)))+
  geom_point(size=0.5) +
  scale_color_manual(values = rep(c("black", "grey"), length(levels(as.factor(merge_sub$scaff_number)))/2))+
  ylab("rho")+
  xlab("Genomic coordinates")+
  theme(
    axis.ticks.x = element_blank(),
...
model<-lm(TRprop ~ log10(median_rho) * chr, data = merge_sub)
summary(model)

cor.test(merge_sub$median_rho, merge_sub$TRprop, method = "spearman")

###Linear Model and correlation on fitted genomic landscapes

gamrho<-gam(median_rho ~ s(cumul_start, bs = "cs", k=30), data=merge_sub)
gamTR<-gam(TRprop ~ s(cumul_start, bs = "cs", k=30), data=merge_sub)

predicted_values_gamrho <- data.frame(cumul_start = merge_sub$cumul_start, 
                                      fitted = predict(gamrho))
predicted_values_gamTR <- data.frame(cumul_start = merge_sub$cumul_start, 
                                     fitted = predict(gamTR))


model<-lm(predicted_values_gamTR$fitted~predicted_values_gamrho$fitted)
summary(model)
...
data_GC2<-data_GC[c(1,2,3,13)]
colnames(data_GC2)<-c("chromosome", "start", "end", "perc_GC")
...
gamGC<-gam(perc_GC ~ s(cumul_start, bs = "cs", k=30), data=merge2)
gamTR<-gam(TRprop ~ s(cumul_start, bs = "cs", k=30), data=merge2)

predicted_values_gamGC <- data.frame(cumul_start = merge2$cumul_start, 
                                      fitted = predict(gamGC))
predicted_values_gamTR <- data.frame(cumul_start = merge2$cumul_start, 
                                     fitted = predict(gamTR))


model<-lm(predicted_values_gamTR$fitted~predicted_values_gamGC$fitted)
summary(model)

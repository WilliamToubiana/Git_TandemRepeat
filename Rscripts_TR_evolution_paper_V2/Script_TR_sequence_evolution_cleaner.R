...
#merge<-merge(mergeTps, presence_absence_df_Tps, by.x="motif", by.y="aomfi", all.x =T)
...
mergeTps$window_start <- (floor(mergeTps$start / 100000) * 100000)+1
mergeTps$window_end <- mergeTps$window_start + 99999  # End of the 100kb window
...
## Plot proportion of Tps unique motifs along chromosomes (per 100kb window)
...
data_GC$window_start <- (floor(data_GC$start / 100000) * 100000)+1
data_GC$window_end <- data_GC$window_start + 99999  # End of the 100kb window
data_GC$chm_pos<-paste(data_GC$chromosome, data_GC$window_start, sep="-")
...
combined<-merge(mergeTRprop, window_summary[3:10], by="chm_pos", all.x=T)
summary(combined)
combined[is.na(combined)] <- 0 #NAs are retrieved when windows have no mapped reads or no TR annotation
combined$avg_shared_corrected<-combined$avg_shared/combined$total_repeats
...
combined3<-merge(combined2, rho_g[3:4], by="chm_pos", all.x = T)
summary(combined3)

##remove windows without recombination estimates
combined3_subset<-na.omit(combined3)
summary(combined3_subset)
colnames(combined3_subset)[14]<-"proportion_species_specific"

##Combine TR prop, centromere, shared repeat, recombination and GC estimates datasets
combined4<-merge(combined3, data_GC[c(4:7)], by="chm_pos", all.x = T)
summary(combined4)
colnames(combined4)[14]<-"proportion_species_specific"
...
combined4_subset<-subset(combined4, chr=="Tps_LRv5b_scf1")
combined4_subset2<-subset(combined4_subset, avg_shared>0)
combined4_subset2<-subset(combined4_subset2, TRprop>0.1)

scatterPlot <- ggplot(combined4_subset2,aes(TRprop, avg_shared, color=centromere)) + 
  geom_point() + 
...
xdensity <- ggplot(combined4_subset2, aes(TRprop, fill=centromere)) + 
  geom_density(alpha=.5) + 
  scale_fill_manual(values = c('#999999','#E69F00')) + 
  theme(legend.position = "none")

# Marginal density plot of y (right panel)
ydensity <- ggplot(combined4_subset2, aes(avg_shared, fill=centromere)) + 
  geom_density(alpha=.5) + 
  scale_fill_manual(values = c('#999999','#E69F00')) + 
...
ggplot(combined4_subset2, aes(x=centromere, y=avg_shared)) + 
  geom_violin()+
  ylab("#Species sharing TR sequences")+
  theme_minimal()+
  theme(axis.text=element_text(size=15),
        axis.title=element_text(size=15))+ geom_boxplot(width=0.2)


##Scatterplot and density plots with recombination
##################################################
combined3_subset2<-combined3_subset
combined3_subset2<-subset(combined3_subset, median_rho<=0.0011)

scatterPlot <- ggplot(combined3_subset2,aes(median_rho, avg_shared, color=centromere)) + 
  geom_point() + 
  scale_color_manual(values = c('#999999','#E69F00')) +
  geom_smooth(method=lm) +
...
# Marginal density plot of x (top panel)
xdensity <- ggplot(combined3_subset2, aes(median_rho, fill=centromere)) + 
  geom_density(alpha=.5) + 
  scale_fill_manual(values = c('#999999','#E69F00')) + 
  theme(legend.position = "none")

# Marginal density plot of y (right panel)
ydensity <- ggplot(combined3_subset2, aes(avg_shared, fill=centromere)) + 
  geom_density(alpha=.5) + 
  scale_fill_manual(values = c('#999999','#E69F00')) + 
  theme(legend.position = "none")+coord_flip()
...
###############################################
combined4_subset2<-subset(combined4_subset2, chr=="Tps_LRv5b_scf1")
combined4_subset2<-subset(combined4_subset2, perc_GC>0.3 & perc_GC<0.5)

scatterPlot <- ggplot(combined4_subset2,aes(perc_GC, avg_shared, color=centromere)) + 
  geom_point() + 
  scale_color_manual(values = c('#999999','#E69F00')) +
  geom_smooth(method=lm) +
  theme(legend.position=c(0,0), legend.justification=c(2,1))+ xlim(0.3, 0.5)+ ylim(1, 5)

# Marginal density plot of x (top panel)
xdensity <- ggplot(combined4_subset2, aes(perc_GC, fill=centromere)) + 
  geom_density(alpha=.5) + 
  scale_fill_manual(values = c('#999999','#E69F00')) + 
  theme(legend.position = "none")

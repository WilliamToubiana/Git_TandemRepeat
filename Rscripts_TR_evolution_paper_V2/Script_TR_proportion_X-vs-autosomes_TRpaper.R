...
merge3<-merge(window_summary_chr, rho_g2, by.x="chr", by.y="scaffold")
...
ggplot(data=merge4, aes(x=log10(median_rho), y=log10(mean_CN))) +
  geom_point(stat="identity", color="black", fill="white")+
  geom_text(aes(label=LGs), vjust=1, size=5.5)+ ylab("mean copy number (log)")+ xlab("rho values (log)")+
  theme_classic(base_size = 17)+
  geom_smooth(method='lm')

model<-lm(log10(merge4$mean_CN)~log10(merge4$median_rho))
summary(model)


ggplot(data=merge4, aes(x=log10(median_rho), y=avg_motif)) +
  geom_point(stat="identity", color="black", fill="white")+
  geom_text(aes(label=LGs), vjust=1, size=5.5)+ ylab("mean TR sequence length")+ xlab("rho values (log)")+
  theme_classic(base_size = 17)+
  geom_smooth(method='lm')

model<-lm(merge4$avg_motif~log10(merge4$median_rho))
summary(model)



ggplot(data=merge4, aes(x=log10(median_rho), y=repeat_density)) +
  geom_point(stat="identity", color="black", fill="white")+
  geom_text(aes(label=LGs), vjust=1, size=5.5)+ ylab("TR array density")+ xlab("rho values (log)")+
  theme_classic(base_size = 17)+
  geom_smooth(method='lm')

model<-lm(merge4$repeat_density~log10(merge4$median_rho))
summary(model)

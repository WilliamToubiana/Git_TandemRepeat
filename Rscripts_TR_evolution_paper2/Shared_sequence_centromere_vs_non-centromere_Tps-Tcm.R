...
#matching the threshold to Tps (i.e., log2cenH3_norm>0.25), leads to similar interpretations.
...
window_summary <- mergeTps2 %>%
  ...
  summarise(
    ...
    shared_repeats = sum(match == "1"),
    non_shared_repeats = sum(match == "0"),
...
stats1 <- window_summary2 %>%
  group_by(centromere) %>%
  summarise(
    mean = mean(TRprop, na.rm = TRUE),
    median = median(TRprop, na.rm = TRUE))

stats2 <- window_summary2 %>%
  group_by(centromere) %>%
  summarise(
    mean = mean(prop_shared, na.rm = TRUE),
    median = median(prop_shared, na.rm = TRUE))
...
scatterPlot <- ggplot() +
  ...
  ylab("Prop shared TR sequences")+  xlab("TR proportion")


# Marginal density plot of x (top panel)
xdensity <- ggplot(window_summary2,
...
ydensity <- ggplot(window_summary2,
                   aes(prop_shared, fill = centromere)) +
...
window_summary2$centromere <- as.factor(window_summary2$centromere)
window_summary2$centromere <- relevel(
  window_summary2$centromere,
  ref = "non-centromere")

n <- nrow(window_summary2)
window_summary2$proportion_shared2 <- (window_summary2$prop_shared * (n - 1) + 0.5) / n #shifts "0" into tiny positive values and "1" into slightly below 1 values 

library(glmmTMB)
model <- glmmTMB(proportion_shared2 ~ centromere * TRprop + (1|chr),
                 family = beta_family(),
                 data = window_summary2)
...
window_summary2$TRcat <- cut(
  window_summary2$TRprop,
  breaks = c(0, 1/3, 2/3, 1),
  labels = c("low", "moderate", "high"),
  include.lowest = TRUE)

ggplot(window_summary2,
       aes(x = TRcat,
           y = proportion_shared2,
           fill = centromere)) +
  geom_boxplot(outlier.alpha = 0.2,
               position = position_dodge(width = 0.8)) +
  scale_fill_manual(values = c('black','red'))+
  labs(x = "Categories of TR proportion",
       y = "Prop shared TR sequences",
       fill = "Region") +
  theme_bw()
...
ggplot(window_summary2,
  ...
  scale_fill_manual(values = c('black','red'))+
  labs(x = "Categories of TR proportion",
       y = "Number of arrays with shared sequences",
       fill = "Region") +
  theme_bw()
...
ggplot(window_summary2,
       aes(x = TRcat,
           y = non_shared_repeats,
           fill = centromere)) +
  geom_boxplot(outlier.alpha = 0.2,
               position = position_dodge(width = 0.8)) +
  scale_fill_manual(values = c('black','red'))+
  labs(x = "Categories of TR proportion",
       y = "Number of arrays with T. poppense-specific sequences",
       fill = "Region") +
  theme_bw()
...
p2 <- ggplot(res_df, aes(x = non_mean_prop)) +
  ...
  ggtitle("Distribution of mean(Proportion shared) for sampled non-centromere sets") +
